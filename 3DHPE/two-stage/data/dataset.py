...
from utils.pose_utils import fliplr_joints, transform_joint_to_other_db, pixel2cam, rigid_align
...
class DatasetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            adj = joints_have_depth
            joints_have_depth = np.array([joints_have_depth]).astype(np.float32)
            
            kpt_2d = joint_img.copy()
            kpt_2d[:,0] = joint_img[:,0] / cfg.output_shape[1] * bbox[2] + bbox[0]
            kpt_2d[:,1] = joint_img[:,1] / cfg.output_shape[0] * bbox[3] + bbox[1]
            if adj:
                f = data['f']
                c = data['c']
                gt_3d_root = data['root_cam']
                kpt_2d[:,2] = (joint_img[:,2] * 2 - 1) * (cfg.bbox_3d_shape[0]/2) + gt_3d_root[2]
                joint_cam = pixel2cam(kpt_2d, f, c).astype(np.float32) 
                joint_cam = joint_cam - joint_cam[self.root_idx]
            else:
                joint_cam = np.zeros_like(kpt_2d)

            heat_x = np.zeros((len(joint_img),cfg.output_shape[1]), dtype=np.float32)
            heat_y = np.zeros((len(joint_img),cfg.output_shape[0]), dtype=np.float32)  

            for joint_id in range(len(joint_img)):
                mu_x = joint_img[joint_id][0] 
                mu_y = joint_img[joint_id][1]
                x = np.arange(0, cfg.output_shape[1], 1, np.float32)
                y = np.arange(0, cfg.output_shape[0], 1, np.float32)
                heat_x[joint_id] = (np.exp(- ((x - mu_x) ** 2) / (2 * cfg.sigma ** 2)))/(cfg.sigma*np.sqrt(np.pi*2)).astype(np.float32)
                heat_y[joint_id] = (np.exp(- ((y - mu_y) ** 2) / (2 * cfg.sigma ** 2)))/(cfg.sigma*np.sqrt(np.pi*2)).astype(np.float32)
            
            return img_patch, joint_img, joint_vis, joints_have_depth, joint_cam, heat_x, heat_y
        else:
            img_patch = self.transform(img_patch)
            return img_patch
