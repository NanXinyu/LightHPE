    ...
    for epoch in range(args.model_epoch[0], args.model_epoch[1]):
        ...
        with torch.no_grad():
            for itr, input_img in enumerate(tqdm(tester.batch_generator)):
                ...
                # forward
                cam_out, coord_out = tester.model(input_img)

                if cfg.flip_test:
                    flipped_input_img = flip(input_img, dims=3)
                    flipped_cam_out, flipped_coord_out = tester.model(flipped_input_img)
                ...
                if vis:
                    ...
                    cv2.imwrite('./output/vis/' + filename + '_output.jpg', tmpimg)
                
                cam_out = cam_out.cpu().numpy()
                cam_preds.append(cam_out)
                coord_out = coord_out.cpu().numpy()
                img_preds.append(coord_out)

        # evaluate
        cam_preds = np.concatenate(cam_preds, axis=0)
        img_preds = np.concatenate(img_preds, axis=0)
        tester._evaluate(img_preds, cam_preds, cfg.result_dir)
