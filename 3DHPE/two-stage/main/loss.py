...
class KLDiscretLoss(nn.Module):
    ...
    def forward(self, outputx, outputy, targetx, targety, target_weight):
        num_joints = outputx.size(1)
        loss = 0

        for idx in range(num_joints):
            coord_predx = outputx[:,idx].squeeze()
            coord_predy = outputy[:,idx].squeeze()
            coord_gtx = targetx[:,idx].squeeze()
            coord_gty = targety[:, idx].squeeze()
            weight = target_weight[:,idx].squeeze()
            loss += (self.criterion(coord_predx,coord_gtx).mul(weight).mean()) 
            loss += (self.criterion(coord_predy,coord_gty).mul(weight).mean()) 
